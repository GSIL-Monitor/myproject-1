...
class TenpayRequestHandler
{
    ...
    {
        $this->TenpayRequestHandler();
    }

    function TenpayRequestHandler()
    {
     ...
     * 初始化函数。
     */
    function init($appid, $appsecret, $partnerkey, $appkey, $token = '')
    {
        $this->debugInfo = '';
        $this->Token = $token;
        $this->app_id = $appid;
    ...
    {
        $client = new TenpayHttpClientHandler();
    ...
    function sendPrepay($packageParams)
    {
        ...
        $tk = json_decode($json);
        // echo "aaaaaaaaaaaaaaaaaaa".$json."ccccccccccccccccccccc<br>";exit;
        if($tk->errcode == 0)
        {
