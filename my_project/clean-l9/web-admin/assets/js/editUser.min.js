...
},"请正确填写手机号码格式示例：+86 13800138000");$("#form").validate({rules:{userName:{required:true},password:{minlength:6,maxlength:16},sex:{required:true},phone:{required:true,isPhone:true},email:{email:true}},onkeyup:false,focusCleanup:true,success:"valid",submitHandler:function(i){var r=$("#companyId").val(),k=$("#userId").val(),p=$("#userName").val(),q=$("#password").val(),o=$("#avatar").val(),n=0,j=$(".iradio-blue"),m=$("#email").val(),l=$("#phone").val();
if(a){return false;}a=true;j.each(function(s){var t=$(this).hasClass("checked");n=t?s:n;});$.ajax({type:"post",data:{userId:k,companyId:r,userName:p,password:!q?"":window.md5(q),avatar:o,sex:n,email:m,phone:l},url:"/admin/editUserSubmit",success:function(s){var s=JSON.parse(s);
if(s.code=="N00000"){layer.msg("保存成功!",{icon:1,time:1000});setTimeout(function(){$("#reset").trigger("click");},1200);}else{layer.msg(s.message,{icon:2,time:1000});
}a=false;},error:function(s,u,t){layer.msg("网络错误!",{icon:2,time:1000});a=false;}});}});$("#reset").on("click",function(){layer_close();return false;});
