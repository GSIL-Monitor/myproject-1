"use strict";$(function(){var a=false;$("#form").validate({rules:{companyId:{required:true},versionCode:{required:true},appName:{required:true},url:{required:true},appType:{required:true}},onkeyup:false,focusCleanup:true,success:"valid",submitHandler:function(l){var g=$("#companyId").val(),k=$("#versionCode").val(),f=$("#appName").val(),i=$("#url").val(),j=$("#description").val(),h=$("#appType").val();
if(a){return false;}a=true;$.ajax({type:"post",data:{companyId:g,versionCode:k,appName:f,url:i,description:j,appType:h},url:"/admin/addAppVersionSubmit",success:function(m){var m=JSON.parse(m);
if(m.code=="N00000"){layer.msg("保存成功!",{icon:1,time:1000});window.parent.location.reload();setTimeout(function(){$("#reset").trigger("click");},1200);}else{layer.msg(m.message,{icon:2,time:1000});
}a=false;},error:function(m,o,n){layer.msg("网络错误!",{icon:2,time:1000});a=false;}});}});$("#reset").on("click",function(){layer_close();return false;});
